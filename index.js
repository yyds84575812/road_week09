...
function showCurrentChainToken() {
    console.log(`1231`)
    if (typeof chainId !== "undefined") {
        console.log(`func chainId = ${chainId}`)
        // Create token list for modal
        let parent = document.getElementById("token_list");
        for (childNode of parent.childNodes) {
            parent.removeChild(childNode);
        }
        for (const i in tokens){
            // Token row in the modal token list
            let token = tokens[i];
            console.log(`token = ${token.chainId}`)
            let chain_id = tokens[i].chainId;
            if (chainId !== chain_id) {
                continue;
            }
            console.log(`chain id equal`)
            let div = document.createElement("div");
            div.className = "token_row";
            let html = `
            <img class="token_list_img" src="${tokens[i].logoURI}">
...
async function connect() {
    if (typeof window.ethereum !== "undefined") {
        try {
            ...
            console.log(`etherum resp = ${resp}`);
            console.log(`etherumn = ${ethereum}`);
            console.log(`type etherum = ${typeof ethereum}`);
